...
The declarative base model passed into ``Database`` (or autogenerated when excluded) has its metadata available at :attr:`sqlservice.Database.metadata`. There are several ``metadata`` based methods available.
