...
# def x_test_session__debug(db: Database):
#     users = create_users(db)
#     user = users[0]
#
#     with db.session() as session:
...
#
#
# def x_test_session(db: Database):
#     db.engine.echo = True
#
#     with db.begin() as session:
#         session.bulk_insert_mappings(User, [{"name": "a"}, {"name": "a"}])
#         # session.bulk_insert_mappings is the same as this...
#         session.execute(User.__table__.insert().values(), [{"name": "a"}, {"name": "a"}])
#
#         # this is different
#         # session.execute(AModel.__table__.insert().values([{"name": "a"}, {"name": "a"}]))
#
#     with db.begin() as session:
#         model1 = User(id=1)
#         session.bulk_save_objects([model1])
#
#     with db.session() as session:
#         model2 = User(id=2)
#         session.bulk_save_objects([model2])
#         session.commit()
#         model2.name = "foobar"
#         session.bulk_save_objects([model2])
#         session.commit()
